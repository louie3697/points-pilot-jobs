"""Standalone Turkish Airlines Miles&Smiles browser scrape for the points-pilot-jobs runner.
...
award routes over a near-term date window via one warmed Chrome session, normalizes, and upserts
into MotherDuck `flights`, then exits. Manual workflow_dispatch or daily cron. Tunable via env:
TURKISH_SCRAPE_DAYS (default 3); single-route on-demand mode via TURKISH_ROUTE_ORIGIN/DEST/DATES.
"""
...
# Popular US gateways ↔ Istanbul (both directions scraped). The availability API returns the
# full itinerary (incl. the IST connection for onward awards), but US↔IST anchors the coverage.
...
# Cron sharding: split TURKISH_ROUTES across N parallel runs on separate runner IPs (defaults to
# a single unsharded run). Single-route on-demand mode ignores sharding (only shard 0 works).
...
def _parse_dates_csv(csv: str) -> list[date]:
    ...
    for tok in csv.split(","):
        ...
        try:
            ...
            logger.warning("ignoring invalid date %r in TURKISH_ROUTE_DATES", tok)
    return out
...
def _build_plan(
    ...
    for origin, dest in TURKISH_ROUTES[shard_index::shards]:
...
def _ping_heartbeat() -> None:
    if not TURKISH_HEARTBEAT_URL:
        return
    try:
        urllib.request.urlopen(TURKISH_HEARTBEAT_URL, timeout=10).close()
    except Exception as exc:  # noqa: BLE001 — monitoring must never break the run
        logger.warning("heartbeat ping failed: %s", exc)


def _turkish_freshness() -> dict:
    try:
        from db.connection import get_connection

        total, newest = (
            get_connection()
            .execute("SELECT count(*), max(scraped_at_utc) FROM flights WHERE source = 'turkish'")
            .fetchone()
        )
        age_h = None
        ...
        return {"turkish_rows": int(total or 0), "turkish_newest_age_h": age_h}
...
def main() -> None:
    ...
    _ping_heartbeat()
    logger.info(
        "=== done — %d Turkish records upserted (routes=%d errors=%d blocked=%s) in %ss ===",
