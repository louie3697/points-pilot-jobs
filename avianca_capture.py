...
Dumps cap_avianca_full.json + cap_avianca_net.json (artifacts) and a log-readable summary to stdout.
"""
...
WARM_URL = "https://www.lifemiles.com/fly/find"
ORIGIN_CITY, ORIGIN_CODE = "New York", "JFK"
DEST_CITY, DEST_CODE = "Bogota", "BOG"
...
async def _av_state(tab, stage):
    ...
    try:
        print(f"[AV {stage}] " + str(await tab.evaluate(js))[:700], flush=True)
    except Exception as e:
        print(f"[AV {stage}] err {str(e)[:60]}", flush=True)


async def drive_avianca(tab):
    ...
    for w in range(8):
        ...
        print(f"[AV WAIT {(w+1)*5}s] {cnt}", flush=True)
        try:
            import json as _j
    ...
    print(f"[AV PROBE] {str(probe)[:900]}", flush=True)
    ...
    try:
        ...
        if isinstance(wh, str) and wh != 'no-form':
            with open('cap_avianca_widget.html', 'w') as fh:
                fh.write(wh[:600000])
    ...
    # LifeMiles is an all-miles award engine (no cash/miles toggle). Pick one-way, fill From+To,
    # pick a date, search. Discovery-instrumented.
    await click_exact(tab, "one way", "one-way")
    await tab.sleep(1)
    await _av_state(tab, "after-oneway")
...
async def main():
    ...
    port = free_port()
    profile = tempfile.mkdtemp(prefix="avcap_")
    ...
    try:
        ...
        try:
            ...
            if isinstance(html, str):
                with open("cap_avianca_results.html", "w") as f:
        ...
        try:
            ...
            if isinstance(cards_html, str):
                with open("cap_avianca_cards.json", "w") as f:
                    f.write(cards_html)
    ...
    with open("cap_avianca_full.json", "w") as f:
        json.dump(cap, f)
    analyze(cap)

    # CDP Network-layer capture (catches service-worker / SSR responses)
    with open("cap_avianca_net.json", "w") as f:
